...
export default function NewPage() {
  ...
  // NOVO: Estado para o conteúdo do editor
  ...
  // Função fictícia de salvar para testar
  const handleSave = () => {
    console.log({
        title,
        slug,
        type,
        isActive,
        content // Aqui está o HTML completo do editor
    });
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p className="text-sm text-slate-500">Crie uma nova página institucional para o site.</p>
        </div>
        <div className="hidden md:flex gap-2">
            ...
            <Button onClick={handleSave} className="bg-primary hover:bg-primary/90">
                <Save className="w-4 h-4 mr-2" /> Salvar Página
            </Button>
        </div>
      </div>
            ...
            <AdminCard title=":: Informações Básicas">
                <div className="space-y-4">
                    <div className="space-y-2">
                        <Label>Título da Página (Nome no Menu) <span className="text-red-500">*</span></Label>
                        <Input 
                            placeholder="Ex: Horários de Missa" 
                            value={title}
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-[10px] text-slate-400">Gerado automaticamente.</p>
                    </div>
                </div>
            </AdminCard>

            {/* AQUI ESTÁ O EDITOR REAL AGORA */}
        ...
        <div className="space-y-6">
            <AdminCard title=":: Publicação">
                <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 mb-4">
                    <span className="text-sm font-medium text-slate-700">Status da Página</span>
                    <div 
                        className={cn(
                ...
                <div className="space-y-4">
                    ...
                    {type === "submenu" && (
                        <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                            <Label className="text-primary">Menu Pai:</Label>
                            <select className="flex h-10 w-full items-center justify-between rounded-md border border-primary/30 bg-blue-50/50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                                <option value="">Selecione o pai...</option>
                                <option value="1">A Paróquia</option>
                                <option value="2">Secretaria</option>
                                <option value="3">Pastorais</option>
                            </select>
                            <p className="text-[10px] text-slate-500">Esta página aparecerá dentro do menu selecionado acima.</p>
                        </div>
                    )}
                </div>
            </AdminCard>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <Button onClick={handleSave} className="w-full h-12 text-base font-bold bg-primary hover:bg-primary/90 shadow-lg">
          <Save className="w-5 h-5 mr-2" /> Salvar Página
        </Button>
      </div>
